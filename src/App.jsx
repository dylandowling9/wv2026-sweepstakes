...
function PinModal({ onUnlock, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:20,zIndex:3000}}>
      <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,color:"#C9A84C",letterSpacing:3}}>ADMIN LOGIN</div>
      <div style={{color:"#666",fontSize:13}}>Enter your PIN to manage the sweepstakes</div>
      <input
        type="password"
        maxLength={4}
        value={val}
        ...
        autoFocus
        placeholder="PIN"
        style={{padding:"14px 20px",fontSize:24,textAlign:"center",letterSpacing:8,background:"#1a1a1a",border:error?"2px solid #d55b5b":"2px solid #333",borderRadius:10,color:"#fff",width:160,outline:"none"}}
      />
      {error && <div style={{color:"#d55b5b",fontSize:13,marginTop:-10}}>Incorrect PIN</div>}
      <button onClick={submit} style={{padding:"12px 40px",background:"linear-gradient(135deg,#C9A84C,#e8c46a)",color:"#111",border:"none",borderRadius:8,fontFamily:"'Bebas Neue',sans-serif",fontSize:20,letterSpacing:2,cursor:"pointer"}}>
        UNLOCK
      </button>
      <button onClick={onClose} style={{color:"#444",background:"none",border:"none",cursor:"pointer",fontSize:13}}>Cancel</button>
    </div>
  );
}
